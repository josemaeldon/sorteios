...
const DrawTab: React.FC = () => {
  ...
  useEffect(() => {
    if (sorteioAtivo) {
      // Get sold tickets (vendidas)
      const soldTickets = cartelas
  ...
  if (!sorteioAtivo) {
    return (
      <div className="text-center py-12">
        ...
        <h2 className="text-2xl font-bold text-foreground mb-2">Sorteio</h2>
        <p className="text-muted-foreground">Selecione um sorteio para iniciar o sorteio</p>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Cartelas vendidas: {availableNumbers.length} | Sorteadas: {drawnNumbers.length} | Restantes: {remainingNumbers.length}
          </p>
        </div>
        ...
        <Card className="border-yellow-500 bg-yellow-50 dark:bg-yellow-950">
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-yellow-800 dark:text-yellow-200">Nenhuma cartela vendida</p>
                <p className="text-sm text-yellow-700 dark:text-yellow-300">É necessário ter cartelas vendidas para realizar o sorteio</p>
              </div>
            </div>
          </CardContent>
        </Card>
              ...
              <div className="flex items-center justify-center min-h-[400px]">
                {currentNumber !== null ? (
                  ...
                  <div className="text-center text-muted-foreground">
                    ...
                    <p className="text-xl">Clique em "Sortear Número" para começar</p>
                  </div>
                )}
              </div>
            </CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">Cartelas vendidas disponíveis</p>
          </CardContent>
        </Card>
